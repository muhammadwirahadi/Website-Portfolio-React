...
vec2 get_img_uv() {
  ...
  if (containerAspect > imageAspect) {
    // Container is wider than image (contain height, center/bottom width)
    float ratio = imageAspect / containerAspect;
    float x_start = 0.5 - ratio * 0.5;
    img_uv.x = (uv.x - x_start) / ratio;
    img_uv.y = uv.y;
  } else {
    // Container is taller than image (contain width, bottom height)
    float ratio = containerAspect / imageAspect;
    img_uv.x = uv.x;
    img_uv.y = 1.0 - (1.0 - uv.y) / ratio;
  }
  return img_uv;
}
...
vec4 sample_image_smooth(vec2 uv) {
  ...
  return texture2D(u_text_texture, vec2(uv.x, 1.0 - uv.y));
}
